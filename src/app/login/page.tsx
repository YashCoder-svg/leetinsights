...
export default function LoginPage() {
  const { user, loading, isMock, loginWithEmail, loginWithGoogle } = useAuth();
      ...
      <div className="w-full max-w-md space-y-8">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">
            Understand your coding journey. Log in to your account.
          </p>
        </div>
        ...
        >
          ...
          {isMock && (
            <div className="mb-6 flex gap-2.5 rounded-lg border border-indigo-500/20 bg-indigo-550/5 px-4 py-3 text-xs text-indigo-700 dark:text-indigo-400">
              ...
              <div>
                <span className="font-bold">Simulated Auth Mode Active:</span> Firebase keys not configured. You can use any mock login (e.g., email: <code className="bg-indigo-500/10 px-1 rounded text-orange-500">john.doe@example.com</code>, password: <code className="bg-indigo-500/10 px-1 rounded text-orange-500">password</code>) or sign up a new mock account!
              </div>
            </div>
          )}

          {error && (
            <div className="mb-6 flex items-center gap-2 rounded-lg border border-red-500/20 bg-red-500/5 px-4 py-3 text-sm text-red-600 dark:text-red-450 animate-shake">
            ...
            <div>
              <div className="flex items-center justify-between">
                ...
                <Link
                  ...
                  className="text-xs font-semibold text-orange-500 hover:text-orange-655"
                >
                  Forgot password?
                </Link>
              </div>
